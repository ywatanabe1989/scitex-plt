...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-plt's —
# figrecipe / matplotlib / pandas / Pillow are absent from the SIF. So we
# install THIS checkout + its [all,dev] extras (WITH dependency resolution)
# into a writable --target dir and prepend that on PYTHONPATH. The SIF still
# supplies the heavy shared base (pip/uv, the python interpreters, scitex-dev's
# deps), so only scitex-plt's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_plt-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# any figure-reproducibility test (figrecipe renders the SAME recipe twice and
# compares) could see render1 != render2 → spurious MSE-over-threshold failures.
# One stable dir + a single warm-up below (build the cache ONCE, pre-fork)
# removes the race. (scitex-plt is a figrecipe alias, so figrecipe's matplotlib
# stack is exercised here.)
...
# Install scitex-plt + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-plt's historical bare-uv/pip workflow so a
...
# scitex-plt's own suite is small, but it aliases figrecipe's matplotlib stack;
# each xdist worker is a SEPARATE PROCESS, so matplotlib's global rcParams /
# pyplot state are naturally isolated per worker — the safe way to parallelise
# any matplotlib-touching suite. Consistent with the rest of the self-hosted
# fleet so the script stays uniform across repos.
#
# Worker count: use ALL cores. The matrix legs run on this repo's dedicated
# self-hosted node (spartan-cpu-scitex-plt-01), so there is no co-tenant to
# yield half the box to. nice/ionice (below) handles the "yield to higher-
# priority work if the node is ever shared" concern instead of statically
# reserving half the CPUs. Floor 4. pyproject addopts may carry `-v`; override
# to `-q` here to keep the CI log compact.
...
# loadscope pins an entire MODULE's tests to ONE worker, which serializes any
# large parametrized module on a single core while the rest idle. `load`
# spreads individual (parametrized) cases across ALL workers, which is the right
# default for a figrecipe-style matplotlib suite where pyplot state is reset
# per-test rather than shared via module/session-scoped fixtures.
#
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_plt --cov-report=xml --cov-report=term \
    -p no:cacheprovider
