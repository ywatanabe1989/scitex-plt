...
# (project=scitex-plt, owner=ywatanabe1989, repo=scitex-plt,
#  workflow=pypi-publish-and-github-release-on-tag.yml). It already is — the
# previous releases published via OIDC under the same trusted publisher and
# the same workflow filename; only the *client* changes here (manual OIDC in
# the SIF instead of the Docker action), not PyPI's trust config.
#
...
TMPDIR="/tmp/publish-scitex_plt-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
export TMPDIR
rm -rf "$TMPDIR"
...
echo "PUBLISH-OK: scitex-plt dist/* uploaded to PyPI via manual OIDC trusted publishing"
